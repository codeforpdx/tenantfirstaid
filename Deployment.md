...
- **External artifact immutability**: production external artifacts — most importantly the Google Vertex AI RAG dataset — must not be modified in-place while in use. Follow a copy → modify → update-deployment pattern: create a new corpus version, validate it in staging, then update `VERTEX_AI_DATASTORE` in the production GitHub environment and redeploy. Never mutate the live corpus directly.
...
Server access requires an SSH key added by a server admin. If you don't have access yet, see [Permissions](#permissions).

```bash
# Connect to the production droplet.
ssh <SSH_USER>@<DROPLET_IP>
```

`SSH_USER` and the droplet IP (`URL`) are stored in the GitHub Actions [production environment settings](https://github.com/codeforpdx/tenantfirstaid/settings/environments). Only maintainers with access to those settings can retrieve them. For contributors who need temporary access to investigate an incident, ask a server admin on Discord (`#tenantfirstaid-general`).
...
4. **Check application logs via journalctl** (requires SSH access) to see Python tracebacks and startup errors.
...
| Chatbot returns wrong answers | Stale or incorrect RAG corpus | Check `VERTEX_AI_DATASTORE` in the env file; re-ingest documents if needed (see `backend/scripts/`) |
