...
# Data Flow — how data moves through the evaluation system

## The dataset — the source of truth

The file `dataset-tenant-legal-qa-examples.jsonl` is the authoritative list of test
examples. Every example contains:

- **The question** — exactly what a tenant might type.
- **Context** — city and state, because tenant law varies by jurisdiction.
- **Reference answer** — a human-verified model conversation showing what a correct,
  well-toned response looks like.
- **Key facts** — the legal facts the response must get right.

This file lives in the git repository so that all contributors share the same set of
test cases. Changes to examples should be committed here, not left only in the cloud.
...
How a question flows from the dataset through the chatbot and judge to a stored score.
:::
...
4. LangSmith sends the question, the chatbot's response, and the reference answer to
   the AI judge.
...
The LangSmith online editor is the most convenient way to refine a reference answer
or reword a test question. But edits made in the browser don't automatically flow
back into the git repository. The pull step closes that loop.
...
The round trip: push to the cloud, edit in the browser, pull back, commit.
:::

**The rule:** anything you change in the browser must be pulled back and committed.
The JSONL file is what other contributors see. Run `dataset diff` first to see what
changed before overwriting either side. The CLI for all of this is covered in
