...
│   ├── types/                      # Auto-generated TypeScript types (gitignored) — do not edit manually, re-run `mise run //backend:generate-frontend-assets` (root-qualified) to regenerate (this splices the backend's `uv` onto the frontend's `PATH`)
