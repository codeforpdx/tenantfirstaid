...
- build and browse the backend user guide (needs [Quarto](https://quarto.org), or add `--container`)
  ...
  % mise run docs-serve
  ```
  `docs-lint`, `docs-check-links`, and `docs-proofread` (or `docs-check` for all three) catch missing docstrings, broken links, and spelling/grammar issues; see the [Command Reference](backend/user_guide/08-command-reference.qmd) chapter.
